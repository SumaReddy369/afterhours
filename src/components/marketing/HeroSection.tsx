        ...
        <div className="relative grid gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-16">
          <div>
            ...
            <motion.p
              ...
              className="mt-5 max-w-lg text-lg text-ah-muted"
            >
              For working professionals <span className="text-ah-ink/90">aged 20–30</span> who moved to a new city for a job
              or after school—when your network didn’t move with you. If work is full but evenings feel empty, that’s
              common; it isn’t a personal failure.
            </motion.p>
            <motion.p
              initial={reduce ? false : { opacity: 0, y: 14 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.55, delay: 0.12, ease: easeOut }}
              className="mt-4 max-w-lg text-base text-ah-muted"
            >
              AfterHours matches you into a small pod: the <span className="font-medium text-ah-ink/90">same ~6 people</span>,{" "}
              <span className="font-medium text-ah-ink/90">one weekly slot</span>, <span className="font-medium text-ah-ink/90">six weeks</span>—so
              connection can deepen through repetition, not one brave night out.
            </motion.p>
            <motion.div
              initial={reduce ? false : { opacity: 0, y: 14 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              {["Ages 20–30", "New city", "Not dating", "Not networking", "Not Meetup"].map((t) => (
                <span
                  key={t}
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-6 left-6 right-6 rounded-2xl border border-ah-border/50 bg-ah-card/95 p-4 shadow-lg backdrop-blur-sm">
        ...
        <p className="mt-1 text-xs text-ah-muted">Belonging that compounds—not another crowded mixer</p>
